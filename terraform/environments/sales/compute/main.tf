...
locals {
  ...
  vm_name_mask               = local.parsed_environments_config["sales"]["vm-name-mask"]
  disk_name_mask             = local.parsed_environments_config["sales"]["disk-name-mask"]
  inventory_result_path      = local.parsed_environments_config["sales"]["vms-hosts-inventory-result-path"]
...
module "vm-sales-strl" {
  ...
  description = "STRL-VM-sales-${each.value}"
  preemptible = false
  nat         = false
...
resource "local_file" "vm_ips" {
  ...
  content = templatefile("${path.module}/inventory.tpl", {
    vm_hostnames = concat(
      [for instance in module.vm-sales-strl : instance.hostname]
    )

    vm_ips = concat(
      [for instance in module.vm-sales-strl : instance.internal_ip]
    )
    }
  )
