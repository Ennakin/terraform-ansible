...
locals {
  ...
  vm_name_mask               = local.parsed_environments_config["mock"]["vm-name-mask"]
  disk_name_mask             = local.parsed_environments_config["mock"]["disk-name-mask"]
  inventory_result_path      = local.parsed_environments_config["mock"]["vms-hosts-inventory-result-path"]
  ...
  #   servers_and_disks_space  = local.parsed_servers_and_disks["space"]["mock"]
}
...
# small - небольшой по ресурсам
module "vm-mock-hrl-small" {
...
resource "local_file" "vm_ips" {
  ...
  content = templatefile("${path.module}/inventory.tpl", {
    vm_hostnames = concat(
      [for instance in module.vm-mock-hrl-small : instance.hostname],
    )

    vm_ips = concat(
      [for instance in module.vm-mock-hrl-small : instance.internal_ip],
    )
    }
  )
